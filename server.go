...
func main() {
	...
	ctx = context.WithValue(ctx, "namespaceService", namespace.NewMgoService(db, nsqEventbus))
	...
	serviceInfo := eventbus.ServiceInfo{
		Name:                  "namespace",
		Hostname:              env.GetDefaultEnvVar("PUBLISHED_HOSTNAME", "namespacebackend"),
	...
	nsqEventbus.On("service.up", "namespace", func(msg []byte) error {
